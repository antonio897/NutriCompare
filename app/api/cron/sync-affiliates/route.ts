// app/api/cron/sync-affiliates/route.ts
/**
 * Cron Job para sincronización diaria de Data Feeds de afiliados (Awin, TradeTracker, HSN, Prozis, etc.)
 * Diseñado como Next.js Route Handler protegido por Bearer Token.
 * 
 * Invocación:
 *   GET /api/cron/sync-affiliates
 *   Headers:
...
/**
 * Parser de Feed CSV en memoria / streaming
 */
function parseCsvFeed(csvContent: string, vendorName: string): FeedItem[] {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Split básico respetando comillas simples
...
/**
 * Parser ligero de Feed XML (formato estándar RSS 2.0 / Google Shopping / Awin)
 */
...
/**
 * Procesa y actualiza las ofertas de los proveedores en la base de datos
 */
async function syncFeedOffers(feedItems: FeedItem[]) {
  ...
  for (const item of feedItems) {
    ...
    // Buscar producto por EAN (código de barras unívoco)
    let product = null;
    if (item.ean) {
    ...
    // Upsert en la tabla de VendorOffer
    ...
    // Registrar histórico de precios para gráficas
...
export async function GET(request: NextRequest) {
  ...
  // 2. Lista de Feeds configurados (URLs remotas o endpoints de redes de afiliados)
  const feedsConfig = [
    {
      name: 'HSN Store',
  ...
  try {
    ...
    // 3. Sincronización con Amazon PA-API si están configuradas las credenciales
    if (process.env.AMAZON_ACCESS_KEY && process.env.AMAZON_SECRET_KEY) {
      console.log('[Cron] Iniciando sincronización de Amazon PA-API...');
    ...
    // 4. Enviar notificación gratuita a Discord/Telegram

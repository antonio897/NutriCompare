 ...
 * - Extrae el desglose nutricional completo.
 * - Cruza las ofertas (VendorOffer) seleccionando el PRECIO MÁS BAJO disponible y enlace de afiliado.
 * - Calcula métricas de valor (Coste por dosis y Coste por 100g de proteína pura).
 */
...
/**
 * Consulta de alta eficiencia para comparar hasta 4 productos
...
export async function getComparisonData(productIds: string[]): Promise<ComparedProductSummary[]> {
  // Limitar estrictamente a 4 productos para el comparador de pantalla dividida
  ...
  const products = await prisma.product.findMany({
    ...
    include: {
      ...
      vendorOffers: {
        ...
        orderBy: {
          currentPrice: 'asc', // Ordenar ofertas de menor a mayor precio
        },
        select: {
          vendorName: true,
          currentPrice: true,
  ...
  return products.map((prod) => {
    // La mejor oferta es el primer elemento gracias al orderBy: { currentPrice: 'asc' }
    const best = prod.vendorOffers[0] || null;
    const lowestPrice = best ? best.currentPrice : null;

    // Cálculo de métricas de coste por dosis (ej. 30g de servicio o 5g creatina)
    ...
    if (lowestPrice && lowestPrice > 0) {
      // Estimación sobre envase estándar de 1kg (1000g) o 200 dosis
      const estimatedServings = 33; // ~1kg / 30g dosis estándar
      costPerServing = Number((lowestPrice / estimatedServings).toFixed(2));

      if (protein > 0) {
        // Coste por cada 100g de proteína neta (para comparar calidad real de precio)
